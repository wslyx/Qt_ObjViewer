...
void MainWindow::openTextureObjFile()
{
    QFileDialog *fileDialog = new QFileDialog(this);
    QString objPath,texturePath;

    fileDialog->setAcceptMode(QFileDialog::AcceptOpen);     // 打开文件模式
    fileDialog->setFileMode(QFileDialog::ExistingFile);     // 显示存在的文件
    fileDialog->setViewMode(QFileDialog::Detail);           // 显示详细模式
//    fileDialog->setNameFilter(tr("obj Files(*.obj)"));  // 过滤图片
    fileDialog->setWindowTitle(tr("1,Choose one obj file and open it"));    // 对话框标题

    if(fileDialog->exec() == QDialog::Accepted)
    {
        objPath = fileDialog->selectedFiles()[0];      // 用户选择的文件名

        qDebug() << "selected file:"
                 << objPath;
    }

//    fileDialog->setNameFilter(tr("png Files(*.png)"));  // 过滤图片
    fileDialog->setWindowTitle(tr("2,Choose one texture jpg file and open it"));    // 对话框标题

    if(fileDialog->exec() == QDialog::Accepted)
    {
        texturePath = fileDialog->selectedFiles()[0];      // 用户选择的文件名

        qDebug() << "selected file:"
                 << texturePath;
        this->openglwidget->loadMesh(objPath,texturePath.toStdString());
    }
}
///
...
//    ui->openGLWidget->
    ui->verticalLayout->addWidget(this->openglwidget);
...
{
    this->openTextureObjFile();
}


void MainWindow::on_pushButton_2_clicked()
{
    this->openglwidget->cleanScene();
}
